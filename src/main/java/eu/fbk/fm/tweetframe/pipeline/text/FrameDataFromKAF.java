...
public class FrameDataFromKAF extends RichFlatMapFunction<Tuple2<KAFDocument,Integer>, Tuple2<String, Integer>> {

    @Override
    public void flatMap(Tuple2<KAFDocument,Integer> value, Collector<Tuple2<String, Integer>> out) throws Exception {
        value.f0.getPredicates().forEach(predicate -> {
            ...
            out.collect(new Tuple2<>(
                    String.format("In frame (%d) %s %s", value.f1, predicateRef.toUpperCase(), sb.toString()),
                    value.f1
            ));
        });
    }
